...
class MusicEmbedder:
    ...
    def to_embedding(self):
        ...
        for idx, (album_name, song_name, path) in pbar:
            ...
            inputs = self.processor(dataset[0]["audio"]["array"], sampling_rate=24000,
                               return_tensors="pt")
            ...
            time_reduced_hidden_states = torch.nn.functional.avg_pool1d(all_layer_hidden_states.transpose(1, 2),
                                                      kernel_size=pool_size).transpose(1, 2)
            ...
            self.song_data[idx] = [album_name, song_name, path,
                                   time_reduced_hidden_states.cpu().flatten().tolist()]
